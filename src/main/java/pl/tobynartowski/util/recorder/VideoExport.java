/**
 * Video Export Simple video file exporter. https://funprogramming.org/VideoExport-for-Processing
 *
 * <p>Copyright (c) 2017 Abe Pazos http://hamoid.com
 *
...
public class VideoExport {
    ...
    protected final String ffmpegMetadataComment =
            "Made with " + "Video Export for Processing - https://git.io/vAXLk";
    ...
    /**
     * Constructor, usually called in the setup() method in your sketch to initialize and start the
     * library.
     *
     * <p>Using a default movie file name (processing-movie.mp4).
     *
     ...
     * @param outputFileName The name of the video file to produce, for instance "beauty.mp4"
     * @example basic
     */
     ...
     * @param parent Parent PApplet, normally "this" when called from setup()
     * @param outputFileName The name of the video file to produce, for instance "beauty.mp4"
     * @param img PImage object to export as video (can be a PGraphics, Movie, Capture...)
                ...
                new JSONArray(
                        new StringList(
                                new String[] {
                                    ...
                                    "-f",
                                    "rawvideo", // format rgb raw
                                    "-vcodec",
                                    "rawvideo", // in codec rgb raw
                                    "-s",
                                    "[width]x[height]", // size
                                    "-pix_fmt",
                                    "rgb24", // pix format rgb24
                                    "-r",
                                    "[fps]", // frame rate
                                    "-i",
                                    "-", // pipe input
                ...
                new JSONArray(
                        new StringList(
                                new String[] {
                                    ...
                                    "-y", // overwrite old file
                                    "-i",
                                    "[inputvideo]", // video file path
                                    "-i",
                                    "[inputaudio]", // audio file path
                                    "-filter_complex",
                                    "[1:0]apad", // pad with silence
                                    "-shortest", // match shortest file
                                    "-vcodec",
                                    "copy", // don't reencode vid
                                    "-acodec",
                                    "aac", // aac audio encoding
                                    "-b:a",
                                    "[bitrate]k", // bit rate (quality)
                                    "-metadata",
                                    "comment=[comment]", // comment
        ...
        try {
            File thisJar =
                    new File(
                            ...
                            .getParentFile()
                            .getParentFile();
            settingsPath = thisJar.getAbsolutePath() + File.separator + "settings.json";
            ...
            if (settingsFile.isFile()) {
                ...
                if (o instanceof String) {
                    settings.setJSONArray(SETTINGS_CMD_ENCODE_VIDEO, toJSONArray((String) o));
                }
                // If String, make it JSONArray
                o = settings.get(SETTINGS_CMD_ENCODE_AUDIO);
                if (o instanceof String) {
                    settings.setJSONArray(SETTINGS_CMD_ENCODE_AUDIO, toJSONArray((String) o));
                }
            } else {
    ...
    /**
     * Allow setting a new movie name, in case we want to export several movies, one after the
     * other.
     *
     ...
     *
     * @param crf Video quality. A value between 0 (high compression) and 100 (high quality,
     *     lossless). Default is 70.
     * @param audioBitRate Audio quality (bit rate in kbps). 128 is the default. 192 is very good.
     *     More than 256 does not make sense. Higher numbers produce heavier files.
     */
     ...
     *
     * @param frameRate The frame rate at which the resulting video file should be played. The
     *     default is 30, which is the recommended for online video.
     */
    ...
    /**
     * You can tell VideoExport not to call loadPixels() internally. Use it only if you already call
     * loadPixels() in your program. Useful to avoid calling it twice, which might hurt the
     ...
     * The text file normally contains the output messages from ffmpeg, which may be useful for
     * diagnosing problems. If video is being exported correctly you may want to call
     * videoExport.setDebugging(false) to avoid creating unnecessary files. Optional.
     *
     * @param saveDebugFile Set to false to disable saving the ffmpeg output in a text file
     */
    ...
    /**
     * Advanced. You can use this if you want to change how ffmpeg behaves, for example to add
     * filters, custom encodings, or anything else you could do with ffmpeg in the command line. See
     * https://forum.processing.org/two/discussion/22139/video-export-library-0-1-9 To find the
     * default values find CMD_ENCODE_VIDEO_DEFAULT in
     * https://github.com/hamoid/video_export_processing/blob/master/src/com/hamoid/VideoExport.java
     *
     * @param newSettings An array with all the command line arguments to call to produce a video.
     */
    public void setFfmpegVideoSettings(String[] newSettings) {
        settings.setJSONArray(
                SETTINGS_CMD_ENCODE_VIDEO, new JSONArray(new StringList(newSettings)));
    }
     ...
     * filters, custom encodings, or anything else you could do with ffmpeg in the command line. See
     * https://forum.processing.org/two/discussion/22139/video-export-library-0-1-9 To find the
     * default values find CMD_ENCODE_AUDIO_DEFAULT in
     * https://github.com/hamoid/video_export_processing/blob/master/src/com/hamoid/VideoExport.java
     *
     * @param newSettings An array with all the command line arguments to call to produce a video.
     */
    public void setFfmpegAudioSettings(String[] newSettings) {
        settings.setJSONArray(
                SETTINGS_CMD_ENCODE_AUDIO, new JSONArray(new StringList(newSettings)));
    }

    /**
     * Adds one frame to the video file. The frame will be the content of the display, or the
     * content of a PImage if you specified one in the constructor.
     */
    ...
    /** Make sure ffmpeg is found, then create a process to run it. */
    ...
    /**
     * You could use the returned value to display a time counter, a progress bar or to create
     * periodic motion, for instance by feeding the returned value into the sin() function, and
     * using the result to drive the position of an object.
     *
    ...
    /**
     * Call this method if you know the path to ffmpeg on your computer (advanced).
     *
     * @param path
     */
    ...
    /**
     * Makes the library forget about where the ffmpeg binary was located. Useful if you moved
     * ffmpeg to a different location. After calling this function the library will ask you again
     * for the location of ffmpeg. Optional.
     */
    ...
    /**
     * Called internally by the file selector when the user chooses the location of ffmpeg on the
     * disk.
     *
        ...
        if (img.pixelWidth % 2 == 1 || img.pixelHeight % 2 == 1) {
            err(
                    "Width and height can only be even numbers when using the h264 encoder\n"
                            + "but the requested image size is "
                            + img.pixelWidth
                            + "x"
                            + img.pixelHeight);
        }
                    ...
                    while ((processDetails = allProcesses.readLine()) != null) {
                        ...
                        if (m.find()) {
                            ...
                            wincon.GenerateConsoleCtrlEvent(
                                    Wincon.CTRL_C_EVENT, Integer.parseInt(m.group(1)));
                            break;
                        }
                    }
                } else {
            ...
            if (cmdArgs[i].contains("[")) {
                cmdArgs[i] =
                        cmdArgs[i]
                                ...
                                .replace(
                                        "[output]",
                                        parent.sketchFile(tmpAudioFile).getAbsolutePath());
            }
        }
